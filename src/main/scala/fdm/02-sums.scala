...
/**
  * Scala 3 supports eithers, which are a generic way to store one of two or more pieces of information at 
...
/**
  * Scala 3 supports case classes, which are a generic way to store two or more pieces of 
...
object enum_basics:
   ...
   */
  enum Currency:
    case USD(dollars: Int, cents: Int)
...
/**
  * Like case classes, Scala 3's enums come equipped with useful functionality that all "data 
...
object enum_utilities:
  enum Card:
    case Clubs(points: Int)
    case Diamonds(points: Int)
    case Spades(points: Int)
    case Hearts(points: Int)
...
/**
  * Scala 3's enums can be generic, which means the enum may introduce a type parameter which can 
...
object enum_generics:
  ...
  enum AdvertisingEvent:
    case None
    case Impression(pageUrl: String, data: String)
    case Click(pageUrl: String, elementId: String, data: String)
    case Action(pageUrl: String, actionName: String, data: String)
